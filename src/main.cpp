...
uint256 hashGenesisBlock("0x79ed7f1babc48903e37c04056f6ad4b7f3c1f7b8236ccdabb42fdca8ed4c97eb");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Becoin: starting difficulty is 1 / 2^12
...
{
    // Becoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 210000); // Becoin: 840k blocks in ~1 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Becoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Becoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Becoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Becoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x6caa433b4a1f1798f37854f9893a274f7733f9242162af9525bd9c16580ece83");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times Our Digital Lives Drive a Brick-and-Mortar Boom in Data Centers";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1609203406;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086193186;

        if (fTestNet)
        {
            block.nTime    = 1609203393;
            block.nNonce   = 387287031;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xef2927bf3ccf623129ac9370591001749f517aede4c276361bccdc28065732df"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Becoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BecoinMiner(CWallet *pwallet)
{
    printf("BecoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running BecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BecoinMiner, pwallet));
}
